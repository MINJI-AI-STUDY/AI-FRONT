...
- 절차:
  ...
  7. readable viewport에서는 two-page spread, 좁은 폭에서는 one-page fallback이 보이는지 확인
- 기대 결과:
  ...
  - PDF page/spread 표현은 구현 가능한 범위만 설명한다.

### 4.2 teacher 보조패널 CTA / 대시보드 흐름
- URL: `/teacher/home` → `/teacher/document-dashboard` → `/teacher/channels/:channelId/workspace`
- 사전 조건:
  - 교사 계정 로그인 완료
...
- 절차:
  ...
  2. 좁은 viewport에서는 `두쪽보기` 토글이 disabled 되고 fallback 사유가 보이는지 확인한다.
  3. 넓은 viewport에서는 `두쪽보기` 전환이 가능하고, 현재 모드 이유가 표시되는지 확인한다.
...
- 기대 결과:
  ...
  - 좁은 화면에서는 두쪽보기 대신 단일 페이지 fallback이 보인다.
...
- 절차:
  ...
  8. readable viewport에서는 two-page spread, 좁은 폭에서는 one-page fallback이 보이는지 확인
- 기대 결과:
...
- [ ] PDF viewer가 readable viewport에서는 two-page spread, 좁은 폭에서는 one-page fallback을 보인다.
- [ ] `다음` 버튼이 마지막 페이지에서 disabled 된다.
- [ ] `두쪽보기` 토글이 좁은 viewport에서 fallback 사유와 함께 disabled 된다.
